...
2. Use `subosito/flutter-action@v2` on the stable channel with dependency caching.
3. Run `flutter pub get`, `dart format --output=none --set-exit-if-changed .`, `flutter analyze`, `flutter test`, and `flutter build web --release --no-pub`.
...
- The workflow checks formatting, analyzes code, runs tests, and builds web.
